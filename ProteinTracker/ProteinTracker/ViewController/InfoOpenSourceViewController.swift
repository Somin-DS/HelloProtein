...
class InfoOpenSourceViewController: UIViewController {
    ...
    let openSource = ["Alamofire", "SwiftyJSON", "Realm", "Charts", "TextFieldEffects", "IQKeyboardManagerSwift", "JGProgressHUD"]
...
extension InfoOpenSourceViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            cell.setText(text: openSource[indexPath.row])
        }else {
            cell.setText(text: api[indexPath.row])
        }
        return cell
    }
